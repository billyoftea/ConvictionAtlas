...
let LlmService = class LlmService {
    ...
    getModel() {
        return (this.configService.get('GONGFENG_MODEL') ??
    ...
    resolveToken() {
        return (this.configService.get('GONGFENG_API_KEY') ??
            this.configService.get('LLM_API_KEY') ??
            process.env['GF_IDE_OAUTH_TOKEN'] ??
            '');
    }
    resolveBaseUrl() {
        return (this.configService.get('GONGFENG_BASE_URL') ??
            this.configService.get('LLM_BASE_URL') ??
            'https://copilot.code.woa.com/server/openclaw/copilot-gateway/v1');
    }
    resolveUsername() {
        return (this.configService.get('GONGFENG_USERNAME') ??
            process.env['GF_IDE_USERNAME'] ??
            'angeloxu');
    }
    resolveDeviceId() {
        return (this.configService.get('GONGFENG_DEVICE_ID') ??
...
let SourceIngestionService = class SourceIngestionService {
    ...
    async ingestPolymarket(limit) {
        ...
        for (const market of markets) {
            ...
            const previousPrice = currentPrice - Number(market.oneDayPriceChange ?? 0);
            ...
            const vol = Number(market.volume24hr ?? market.volume24hrClob ?? market.volumeNum ?? 0);
            ...
            const fallbackHistory = [
                {
                    ...
                    price: (0, helpers_1.round)(Math.max(0, Math.min(1, priceMonth)), 6),
                    volume: vol,
                    metadata: (0, helpers_1.serializeJson)({ source: 'implied_30d_ago' }),
                },
                {
                    opportunityId: opportunity.id,
                    pointAt: new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000),
                    price: (0, helpers_1.round)(Math.max(0, Math.min(1, priceWeek)), 6),
                    volume: vol,
                    metadata: (0, helpers_1.serializeJson)({ source: 'implied_7d_ago' }),
                },
                {
                    opportunityId: opportunity.id,
                    pointAt: new Date(now.getTime() - 24 * 60 * 60 * 1000),
                    price: (0, helpers_1.round)(Math.max(0, Math.min(1, previousPrice)), 6),
                    volume: vol,
                    metadata: (0, helpers_1.serializeJson)({ source: 'implied_24h_ago' }),
                },
                {
                    opportunityId: opportunity.id,
                    pointAt: now,
                    ...
                    volume: vol,
                    metadata: (0, helpers_1.serializeJson)({ source: 'current_market' }),
                },
    ...
    async ingestNews(limitPerOpportunity) {
        ...
        if (!activeProviders.length) {
            return {
                ingested: 0,
                skipped: true,
                reason: 'No CRYPTOPANIC_API_KEY, GNEWS_API_KEY, or NEWSAPI_KEY configured.',
...
let QueryService = class QueryService {
    ...
    buildManagerAnalytics(latestPortfolio, latestPerformance, options) {
        const replaySeries = options?.managerSlug
            ? this.buildReplaySeriesForManager(options.managerSlug, latestPortfolio, options.replayUniverse)
            : [];
        const adjustedReplaySeries = options?.managerSlug && replaySeries.length
        ...
        if (!series.length) {
            return {
                latestNav: (0, helpers_1.round)(latestPerformance?.nav ?? latestPortfolio?.nav ?? 100, 4),
                dailyReturn: (0, helpers_1.round)(latestPerformance?.dailyReturn ?? 0, 4),
                cumulativeReturn: (0, helpers_1.round)(latestPerformance?.cumulativeReturn ?? 0, 4),
                drawdown: (0, helpers_1.round)(latestPerformance?.drawdown ?? 0, 4),
                sharpe: (0, helpers_1.round)(latestPerformance?.sharpe ?? 0, 4),
                hitRate: (0, helpers_1.round)(latestPerformance?.hitRate ?? 0, 4),
                lookbackDays: 0,
                series: [
                    {
                        pointAt: new Date().toISOString(),
                        nav: (0, helpers_1.round)(latestPerformance?.nav ?? latestPortfolio?.nav ?? 100, 4),
                        cumulativeReturn: (0, helpers_1.round)(latestPerformance?.cumulativeReturn ?? 0, 4),
                    },
                ],
            };
        ...
        const peakBeforeLatest = series.reduce((max, point) => Math.max(max, point.nav), 100);
        const drawdown = peakBeforeLatest > 0 ? latestNav / peakBeforeLatest - 1 : 0;
        const sharpe = periodReturns.length
            ? (0, helpers_1.round)((0, helpers_1.clamp)((0, helpers_1.standardDeviation)(periodReturns) === 0
                ? (0, helpers_1.average)(periodReturns)
                : (0, helpers_1.average)(periodReturns) / (0, helpers_1.standardDeviation)(periodReturns), -4, 4), 4)
            : 0;
        const hitRate = replaySeries.length
            ? (0, helpers_1.round)((0, helpers_1.clamp)(periodReturns.filter((value) => value > 0).length /
                Math.max(periodReturns.length, 1), 0.35, 0.82), 4)
            : latestPortfolio?.positions?.length
                ? (0, helpers_1.round)(latestPortfolio.positions.filter((position) => {
        ...
        return {
            ...
            dailyReturn: (0, helpers_1.round)(periodReturns[periodReturns.length - 1] ?? 0, 4),
