...
let QueryService = class QueryService {
    ...
    async getLatestManagerState(manager) {
        const [latestPerformance, performanceHistory, latestPortfolio] = await Promise.all([
        ...
        const analytics = this.buildManagerAnalytics(latestPortfolio, latestPerformance, performanceHistory);
        return { latestPerformance, latestPortfolio, analytics };
    }
    buildManagerAnalytics(latestPortfolio, latestPerformance, performanceHistory) {
        const series = this.buildPerformanceSeries(performanceHistory, latestPerformance, latestPortfolio);
        const periodReturns = (performanceHistory ?? [])
            .map((entry) => Number(entry.dailyReturn))
            .filter((value) => Number.isFinite(value));
        ...
        return {
            ...
            dailyReturn: (0, helpers_1.round)(latestPerformance?.dailyReturn ?? periodReturns[periodReturns.length - 1] ?? 0, 4),
            cumulativeReturn: (0, helpers_1.round)(latestPerformance?.cumulativeReturn ?? latestNav / 100 - 1, 4),
            drawdown: (0, helpers_1.round)(latestPerformance?.drawdown ?? drawdown, 4),
            sharpe,
            hitRate,
            lookbackDays,
            series,
