#!/usr/bin/env python3
"""
LLM Engine Core — 工蜂 AI (gongfeng/auto) with OAUTH token
"""
import json, urllib.request, urllib.error

AUTH_PATH = "/projects/.openclaw/agents/main/agent/auth-profiles.json"
GONGFENG_API = "https://copilot.code.woa.com/server/openclaw/copilot-gateway/v1/chat/completions"

def get_auth():
    with open(AUTH_PATH) as f:
        profiles = json.load(f)
    p = profiles["profiles"]["gongfeng:default"]
    return p["access"], p["username"], p["deviceId"]
...
def llm_call(system_prompt: str, user_prompt: str,
    ...
    req = urllib.request.Request(
        GONGFENG_API, data=payload, method="POST",
        headers={
